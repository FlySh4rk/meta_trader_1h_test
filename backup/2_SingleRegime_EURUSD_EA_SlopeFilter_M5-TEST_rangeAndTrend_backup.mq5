...
//--- MODIFICATO: Ottenimento handle del filtro MA
   ...
   if(onnx_handle == INVALID_HANDLE)
     {
      printf("Errore OnnxCreateFromBuffer: %d", GetLastError());
      return(INIT_FAILED);
     }
   ...
   if(!OnnxSetInputShape(onnx_handle, 0, input_shape))
     {
      printf("Errore OnnxSetInputShape: %d", GetLastError());
   ...
   if(!OnnxSetOutputShape(onnx_handle, 0, output_shape))
     {
      printf("Errore OnnxSetOutputShape: %d", GetLastError());
      OnnxRelease(onnx_handle);
      onnx_handle = INVALID_HANDLE;
      return(INIT_FAILED);
     }

   // --- PHASE 1 FIX --- rimossa inizializzazione contatori daily/cooldown
   ...
   if(direction_filter != ALLOW_SHORTS_ONLY && closePrice > donchianUpper)
     {
      ...
      if(lotSize <= 0.0)
      ...
      else
        {
         ...
         int mlPred = GetMLPrediction(true, true);
         if(mlPred == 0)
           {
      ...
      if(direction_filter != ALLOW_LONGS_ONLY && closePrice < donchianLower)
        {
         ...
         if(lotSize <= 0.0)
         ...
         else
           {
            ...
            int mlPred = GetMLPrediction(true, false);
            if(mlPred == 0)
              {
               PrintFormat("ML Model Blocked Trade - Skipped");
               return;
   ...
   if(direction_filter != ALLOW_SHORTS_ONLY && lowPrice <= bbLower[0] && rsiValue < InpRSI_Oversold)
     {
      ...
      if(lotSize <= 0.0)
      ...
      else
        {
         ...
         int mlPred = GetMLPrediction(false, true);
         if(mlPred == 0)
           {
            PrintFormat("ML Model Blocked Trade - Skipped");
            return;
      ...
      if(direction_filter != ALLOW_LONGS_ONLY && highPrice >= bbUpper[0] && rsiValue > InpRSI_Overbought)
        {
         ...
         if(lotSize <= 0.0)
         ...
         else
           {
            ...
            int mlPred = GetMLPrediction(false, false);
            if(mlPred == 0)
              {
               PrintFormat("ML Model Blocked Trade - Skipped");
               return;
...
double CalculateLotSize(const double slDistanceInPrice, const ENUM_ORDER_TYPE type, const double price)
  {
   ...
   double volRisk = CalcVolumeByRisk(slDistanceInPrice, InpRiskPercent);
   if(volRisk <= 0.0)
      return 0.0;

   double volume = CapVolumeByMargin(volRisk, type, price, 0.95);
   return volume;
  }
...
int GetMLPrediction(bool is_trend, bool is_long)
  {
   ...
   if(!OnnxRun(onnx_handle, ONNX_NO_CONVERSION, features, output_label))
     {
      PrintFormat("Errore OnnxRun: %d", GetLastError());
      return 1;
     }
